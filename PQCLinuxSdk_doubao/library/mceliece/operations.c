...
void dump_buf1(char *info, uint8_t *buf, uint32_t len)
{
	printf("%s[%d]", info, len);
	for (int i = 0; i < len; i++) {
...

{
	...
	// unsigned char s1s[32];
	// unsigned char cts[128];
	// crypto_kem_enc(cts, s1s, mceliece_enc_in_ref);
	// dump_buf1("cts", cts, 128);  //c
	// dump_buf1("s1s", s1s, 32);  //c
	// memcpy(c, mceliece_dec_in_ref, 128);
	// // // reverse_every_four_bytes(ct, 128); //ct
	// //  memcpy(sk, mceliece_dec_in_ref+32, 6492);
 int i;

 //dump_buf1("sk", sk, 6492);  //c
 unsigned char ret_confirm = 0;
 unsigned char ret_decrypt = 0;
