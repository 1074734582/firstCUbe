                   ...
                   const unsigned char *pk)
{
  ...
  int kyber_keygen_512_in_ref[16]={
	0xa035997c,	0xaa9476b0,	0xe4106d0c,	0xdd1a6bdb,
	0x251ad82f,	0x0348b1cc,	0x9973cd2d,	0x2d7f7336,
	0x00000000,	0x00000000,	0x00000000,	0x00000000,
	0x00000000,	0x00000000,	0x00000000,	0x00000000,
};
  memcpy(buf, kyber_keygen_512_in_ref, KYBER_SYMBYTES);
...
/***********************************/
int crypto_kem_keypair_user(unsigned char *seed, unsigned char *pk, unsigned char *sk)
{
  size_t i;
  indcpa_keypair_user(seed, pk, sk);
  for(i=0;i<KYBER_INDCPA_PUBLICKEYBYTES;i++)
    sk[i+KYBER_INDCPA_SECRETKEYBYTES] = pk[i];
  hash_h(sk+KYBER_SECRETKEYBYTES-2*KYBER_SYMBYTES, pk, KYBER_PUBLICKEYBYTES);
  /* Value z for pseudo-random output on reject */
...
int crypto_kem_enc_user(unsigned char *seed, unsigned char *ct,
                   unsigned char *ss,
                   const unsigned char *pk)
{
  uint8_t buf[2*KYBER_SYMBYTES];
