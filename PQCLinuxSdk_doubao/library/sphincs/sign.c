...
/****************************zwd**********************************/
int crypto_sign_keypair_user(unsigned char *seed, unsigned char *pk, unsigned char *sk)
{
    unsigned char sd[CRYPTO_SEEDBYTES];

    memcpy(sd, seed, CRYPTO_SEEDBYTES);
//   randombytes(seed, CRYPTO_SEEDBYTES);
    crypto_sign_seed_keypair(pk, sk, seed);
    return 0;
}

int crypto_sign_signature_user(uint8_t *seed, size_t seedlen,uint8_t *sig, size_t *siglen,
                          const uint8_t *m, size_t mlen, const uint8_t *sk)
{
    const unsigned char *sk_seed = sk;
    const unsigned char *sk_prf = sk + SPX_N;
...
int crypto_sign_verify_user(const uint8_t *sig, size_t siglen,
                       const uint8_t *m, size_t mlen, const uint8_t *pk, uint8_t *pkroot)
{
    const unsigned char *pub_seed = pk;
    const unsigned char *pub_root = pk + SPX_N;
    unsigned char mhash[SPX_FORS_MSG_BYTES];
